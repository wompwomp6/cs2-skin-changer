...
void OnWeapon(const uintptr_t& pWeapon)
{
    ...
    if (mem->Read<int32_t>(pWeapon + Offsets::m_nFallbackPaintKit) != Skin.Paint)
    {
        SetMeshMask(pWeapon, 1 + Skin.bUsesOldModel);
        SetMeshMask(hudWeapon, 1 + Skin.bUsesOldModel);

        mem->Write<uint32_t>(item + Offsets::m_iItemIDHigh, ItemIds::UseFallBackValues);
        mem->Write<int32_t>(pWeapon + Offsets::m_nFallbackPaintKit, Skin.Paint);
    }
}
